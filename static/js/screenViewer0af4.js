...
var sV = {
  ...
  init: function () {
    ...
    if (Browser.ie6 || Browser.ie7) {
      $j("#blackout").show();
      $j("#blackout").height($j(document).height());
  ...
  imageIndex: function (direction) {
    if (sV.artGalleryId) {
      if (direction == "prev") {
        return sV.currentIndex + 1 >= sV.imageCollection.length
          ? 0
          : sV.currentIndex + 1;
      } else if (direction == "next") {
        return sV.currentIndex - 1 < 0
          ? sV.imageCollection.length - 1
          : sV.currentIndex - 1;
      }
    } else {
      if (direction == "prev") {
  ...
  setImage: function (src) {
    ...
    if (!image.complete) $j(sV.loader).show();
  ...
  loadVideo: function (name, width, height, clickedLink, title) {
    ...
    var altLocale =
      gameTrailersLoc[name] != "" ? gameTrailersLoc[name] : locale;
    ...
    if (
      ...
      (name == "intro-movie" ||
        name == "blacksoulstone" ||
        name == "teaser" ||
        name == "barbarian-overview" ||
        name == "wizard-overview" ||
        name == "monk-overview" ||
        name == "witch-doctor-overview" ||
        name == "demon-hunter-overview")
    ) {
      ...
      if (
        locale != "en_us" &&
        gameType == "d3" &&
        (name == "intro-movie" || name == "blacksoulstone")
      ) {
        cPath =
          akamaiBaseUrl +
          "blizzard/movies/games/" +
          gameType +
          "/" +
          name +
          "/captions/" +
        ...
        (locale == "es_mx" && gameType == "d3") ||
        (locale == "pt_br" && gameType == "d3") ||
        (locale == "pl_pl" && gameType == "d3")
      ) {
        cPath =
          akamaiBaseUrl +
          "blizzard/movies/games/" +
    ...
    if (ageGateStatus == "prompt" || ageGateStatus == "unavailable") {
      ...
      checkAgeGateVideo(function () {
        swfobject.embedSWF(
          ...
          height,
          "10",
          akamaiBaseUrl + "global-video-player/expressInstall.swf",
          newFlashVars,
          defaultVideoParams,
          attributes
        );
      ...
      if (name == "youtube-reveal") {
        sV.youtubeEmbed(youtube, width, height);
      } else {
        swfobject.embedSWF(
          akamaiBaseUrl +
            "global-video-player/themes/" +
  ...
  youtubeEmbed: function (youtube, width, height) {
    $j("#videoContainer").append(
      $j(
        '<object id="evil" width="' +
          width +
          '" height="' +
          height +
          '"><param name="movie" value="https://www.youtube-nocookie.com/v/' +
          youtube +
          '?enablejsapi=1&version=3&autohide=1&autoplay=1&rel=0&showinfo=0&modestbranding=1&egm=0?enablejsapi=1"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="https://www.youtube-nocookie.com/v/' +
          youtube +
          '?enablejsapi=1&version=3&autohide=1&autoplay=1&rel=0&showinfo=0&modestbranding=1&egm=0&hd=1" type="application/x-shockwave-flash" width=' +
          width +
          ' height="' +
          height +
  ...
  reloadYoutube: function (youtube, width, height, title) {
    $j(".videoThumbnail").removeClass("selected");
    $j(".intro-youtube").addClass("selected");
    $j(".viewer-wrapper").empty().append('<div id="videoContainer">');
    $j("#viewerTitle").html(title);
    sV.youtubeEmbed(youtube, width, height);
  },
  ...
  loadImage: function (image) {
    if (image.complete) {
      if (
        window.location.href.indexOf("fanart") > -1 ||
        window.location.href.indexOf("conceptart") > -1
      ) {
        sV.setCopyUrl();
        sV.setContainerDimensions(
          image.width,
          image.height,
          sV.descImage.height() + 30
        );
      } else {
      ...
      sV.t = window.setTimeout(function () {
        sV.loadImage(image);
      }, 50);
    }
  },
  ...
  setCopyUrlBtn: function (image) {
    ...
    var tCopyLink = $j("<div>")
      .attr("id", "link_text")
      .append($j("<input>").attr("value", sV.imageCopyLinkURL))
      .css("width", image.width - (sV.btnImage.width + 15) + "px");

    var btnCopyUrlDiv = $j("<div>")
      .addClass("btn_copylink")
      .css("width", sV.btnImage.width)
    ...
    var params = {
      menu: "false",
      wmode: "transparent",
      base: Core.staticUrl + "/_flash/",
      allowScriptAccess: "true",
    };
    swfobject.embedSWF(
      Core.staticUrl + "/_flash/_copyurl.swf",
      "btn_copyUrl",
      sV.btnImage.width,
      "21",
      "9.0.0",
  ...
  afterCopyUrl: function () {
    ...
    sV.firstCopySetTimeId = window.setTimeout(function () {
      copyUrlDiv
        .attr("value", GALLERY_MSG_DONECOPY)
        .fadeIn(100)
        .css("color", "#00adef");
    }, 100);
    ...
    sV.thirdCopySetTimeId = window.setTimeout(function () {
      copyUrlDiv
        .attr("value", copyUrlDiv.data("copyUrlData"))
        .css("color", "#227593")
        .fadeIn(200);
    }, 1500);
  },
  ...
  showSubmission: function (submissionNo) {
    // TO DO: fix set pages each to view screendshots
    /*
  ...
  setContainerDimensions: function (width, height, addheight) {
    $j(sV.frame)
      .css("height", height + addheight + "px")
      .css("width", width + "px");
    $j(sV.content)
  ...
  close: function () {
    ...
    var videoContainerWrapper = $j("<div>")
      .addClass("viewer-wrapper")
      .appendTo($j(sV.content));
